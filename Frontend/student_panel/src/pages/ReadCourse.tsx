...
async function getStudentId(){
  const tokenData = localStorage.getItem(ApiConfigs.TOKEN_CREDENTIAL);
      if (tokenData) {
        ...
        setStudentId(parsed.userDetails.uid);
      }
}
...
async function submitRating() {
  ...
  try {
    ...
    if (await AuthenticationService.isAuthenticated()) {
      ...
      let response=await Api("course/rateCourse", {
        courseId,
        rating,
        studentId
      });
      ...
      if(response.status==200){
       ...
       setRatingMessage("⭐ Thank you for rating this course!");
       setHasRated(true)
      }
      else{
    ...
    setRatingMessage("Failed to submit rating. Try again.");
  } finally {
  ...
  return (
    <>
        ...
        >
          <div className="p-4 border-b flex items-center justify-between">
            <h2 className="text-sm font-semibold flex items-center gap-2">
              <TbMessageChatbotFilled size={25} /> Ask Anything About This Course
            </h2>
            <button
              onClick={() => setMessage([])}
      ...
      {!hasRated && ( <div className="w-full flex justify-center mt-6 mb-6">
        <div
          className={`px-6 py-4 rounded-2xl shadow-md flex flex-col items-center gap-3 ${
            darkMode ? "bg-gray-800 text-white" : "bg-white text-gray-800"
          }`}
        >
          <p className="text-sm font-semibold tracking-wide">Rate this course</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`ml-4 px-5 py-1.5 rounded-full text-sm font-semibold transition ${
                rating
